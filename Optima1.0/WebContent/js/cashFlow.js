	...
	if (allResults.result == 0) {
	    ...
	    for (var i = 0; i < projects.length; i++) {
			...
			if (projectCashFlowData && projectCashFlowData.map) {
			 ...
			 /*   var currentNetBalaceRow = rowCount;
			    pData[rowCount] = {};
			    pData[rowCount]["day"] = "Net Balance ";
			    rowCount++;*/
			    ...
			    while (runningDate <= endDate) {
					...
					if (totalCashout[formattedDate1] == null) totalCashout[formattedDate1] = 0;
					if (totalFinanceCost[formattedDate1] == null) totalFinanceCost[formattedDate1] = 0;
...
//						+ projects[i].projectId].financeCost;
					...
					pData[currentBalanceRow][formattedDate1] = data.map[formattedDate2 + ","
						+ projects[i].projectId].balance;
					totalBalance[formattedDate1] +=  data.map[formattedDate2 + "," + projects[i].projectId].balance;
					
					pData[currentPaymentsRow][formattedDate1] = data.map[formattedDate2 + ","
						+ projects[i].projectId].payments;
					
					totalPayments[formattedDate1] += data.map[formattedDate2 + "," + projects[i].projectId].payments;
				    
					/*pData[currentNetBalaceRow][formattedDate1] = data.map[formattedDate2 + ","
						+ projects[i].projectId].netBalance;*/
	   ...
	   /* currentNetBalaceRow = rowCount;
	    ...
	    runningDate = new Date(startDate);
	    while (runningDate <= endDate) {
	    	var formattedDate1 = fmt.format(runningDate);
	...
	var pGrid = new Slick.Grid("#cashFlowGrid", pData, pColumns, {
	    editable : true,
	    enableAddRow : true,
