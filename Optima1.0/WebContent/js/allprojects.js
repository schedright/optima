  ...
  if (allProjects && allProjects.data && allProjects.data.list) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (name && code) {
        var include = true;
        if (search) {
          if (name.toLowerCase().indexOf(search) == -1 && code.toLowerCase().indexOf(search) == -1) {
            include = false;
          }
        }
        if (include) {
  ...
  $('#importsProjects').on('click', function() {
    ...
    fileInput.change(function() {
      if (fileInput[0].files[0]) {
        ...
        /*
         * $.ajax({ url: 'upload.php', type: 'POST', processData: false, // important contentType: false, // important dataType : 'json', data: myFormData });
         */
      }
    });
  ...
  $('#editProject').on('click', function() {
    ...
    if (row) {
      ...
      if (item) {
        ...
        $("#createOrEditProjectDialog").data("project", item).dialog("open");
      }
    }
  });
  $('#deleteProject').on('click', function() {
    ...
    if (row) {
      ...
      if (item && item.proj && item.proj.projectId) {
        ...
        showMessage('Delete Project', 'The selected project will be deleted permanently!', 'warning', buttons);
      }
    }
  });
  ...
  $("#createOrEditProjectDialog").dialog({
    ...
    buttons : {
      "Save" : function() {
        ...
        var bValid = true;
        bValid = bValid && checkLength($("#projectName"), "projectName", 3, 32);
        bValid = bValid && checkLength($("#projectCode"), "projectCode", 3, 32);
        if (bValid) {
          var project = $(this).data("project");
          if (project && project.proj) {
            project = project.proj;
          ...
          if (project && project.portfolio && project.portfolio.portfolioId) {
