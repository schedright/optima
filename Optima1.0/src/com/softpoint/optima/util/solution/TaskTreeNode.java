...
public class TaskTreeNode {
	...
	/**
	 * shift the task by few days. it will clear all the start days of all tasks depending on it and will change all the duration with children of both children and parent so it will just clear them and they will be recalculated the first
	...
	/*
	 * return the calculated task end, it will use the parents time in case they got shifted
	 */
	public Date getCalculatedTaskEnd() {
		if (calculatedTaskEnd == null) {
			...
			for (TaskTreeNode pNode : parents) {
				...
				while (PaymentUtil.isDayOff(e, projectW.getProjectVacations()) || TaskUtil.isWeekendDay(e, projectW.getProjectWeekends())) {
			...
			if (duration > 0) {
				while (duration > 0) {
					if (PaymentUtil.isDayOff(datePointer, projectW.getProjectVacations()) || TaskUtil.isWeekendDay(datePointer, projectW.getProjectWeekends())) {
						calendarDuration++;
					} else {
