...
public class PaymentUtilBeforeSolving extends PaymentUtil {
	...
	public static double getPortfolioLeftOverCost(EntityController<Portfolio> controller , Portfolio portfolio , Date from , Date to, HashMap<Integer, Integer> completedProjects) {
		...
		for (Project project : projects) {
			try {
				Date[] projectDates = PaymentUtilBeforeSolving.getProjectExtendedDateRanges(controller, project.getProjectId());
				Date projectEndDate = projectDates[1];
				
				projectDates = PaymentUtilBeforeSolving.getProjectDateRanges(controller, project.getProjectId());
	...
	public static double getLeftOverCost(Project project , Date from , Date to , Date projectStartDate , Date projectEndDate, boolean projectCompleted) {
		...
		numberOfDays = PaymentUtilBeforeSolving.daysBetween(PaymentUtilBeforeSolving.maxDate(from, projectStartDate) , PaymentUtilBeforeSolving.minDate(to, projectEndDate));
		...
		for (ProjectTask currentTask : tasks) {
			...
			Date taskDate = PaymentUtilBeforeSolving.getTaskDate(currentTask);
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(taskDate);
			calendar.add(Calendar.DATE, calculateTaskDuration(currentTask));
			Date taskEndDate = calendar.getTime();
			
			if (taskDate.before(from) && taskEndDate.after(from)){
				
				int taskDaysAfterPeriodStart = PaymentUtilBeforeSolving.daysBetween(PaymentUtilBeforeSolving.maxDate(from, projectStartDate), taskEndDate);
				...
				int noOfWeekendDaysAndDaysOf = PaymentUtilBeforeSolving.getNoOfWeekEndDaysAndDaysOff(currentTask, from, endEffectiveDate.getTime()); 
...
//		dates[1] = calendar.getTime();
	...
	public static double getDateTasksCashout(Project project, Date currentDate , boolean includeOverHeads ) {
		...
		while (iter.hasNext()) {
			...
			if (taskDate != null && isDateInBetweenDates(taskDate, DateUtils.add(taskDate, Calendar.DATE, calculateTaskDuration(currentTask) - 1), currentDate)
	...
	public static double getProjectPaymentstNew(HttpSession session, Project project, Date currentDate) throws EntityControllerException {
		...
		Date[] projectBoundaries = PaymentUtilBeforeSolving.getPortofolioDateRangesNew(controller, project.getPortfolio().getPortfolioId());
		...
		try {
			...
			for (ProjectTask task : tasks) {
				...
				if (portfolioEndDate == null) {
					...
					Calendar calendar = Calendar.getInstance();
					calendar.setTime(taskDate);
					calendar.add(Calendar.DATE, calculateTaskDuration(task));
	...
	public static Period findPaymentScheduleNew (HttpSession session , Date date, int portfolioId) throws OptimaException {
		...
		try {
			Date[] projectBoundaries = PaymentUtilBeforeSolving.getPortofolioDateRangesNew(controller, portfolioId);
	...
	public static Period findPaymentSchedule(HttpSession session , Date date, int portfolioId) throws OptimaException {
		EntityController<ProjectPayment> controller = new EntityController<ProjectPayment>(session.getServletContext());
		try {
			Date[] projectBoundaries = PaymentUtilBeforeSolving.getPortofolioDateRanges(controller, portfolioId);
			Period boundaries = new Period(projectBoundaries[0] , projectBoundaries[1]); 
...
public static Period findFinanceSchedule(HttpSession session , Date date, int portfolioId) throws OptimaException {
	...
	public static 	Map<String, DailyCashFlowMapEntity> getProjectCashFlowDataNew(HttpSession session, int projectId, Date from, Date to) throws OptimaException {
		...
		try {
			...
			Date[] projectDates = PaymentUtilBeforeSolving.getProjectDateRanges(controller, project.getProjectId());
			Date projectStartDate = projectDates[0];
			Date projectEndDate = projectDates[1];
			...
			for (Date date = start.getTime(); !start.after(end); start.add(Calendar.DATE, 1), date = start.getTime()) {
				...
				entity.setCashout(PaymentUtilBeforeSolving.getDateTasksCashout(project, date , includeOverhead));
				entity.setFinanceCost(PaymentUtilBeforeSolving.getDateFinanceCost(project, date, results));
				entity.setPayments(PaymentUtilBeforeSolving.getProjectPaymentstNew(session,project, date));
				entity.setBalance(PaymentUtilBeforeSolving.getBalance(date, entity, results));
				entity.setNetBalance(entity.getBalance() + entity.getFinanceCost());
				
				
				results.put( PaymentUtilBeforeSolving.dateOnlyFormat.format(date) + "," + project.getProjectId(), entity);
			}
	...
	public static 	Map<String, DailyCashFlowMapEntity> getProjectCashFlowData(HttpSession session, int projectId, Date from, Date to) throws OptimaException {
		...
		try {
			...
			for (Date date = start.getTime(); !start.after(end); start.add(Calendar.DATE, 1), date = start.getTime()) {
				...
				entity.setCashout(PaymentUtilBeforeSolving.getDateTasksCashout(project, date , includeOverhead));
				entity.setFinanceCost(PaymentUtilBeforeSolving.getDateFinanceCost(project, date, results));
				entity.setPayments(PaymentUtilBeforeSolving.getProjectPaymentst(project, date));
				entity.setBalance(PaymentUtilBeforeSolving.getBalance(date, entity, results));
				entity.setNetBalance(entity.getBalance() + entity.getFinanceCost());
	...
	public static List<ProjectTask> getEligibleTasks(Project project, Date from, Date to , boolean useCalDate) {
		...
		for (ProjectTask currentTask : projectTasks) {
			Date taskDate = PaymentUtilBeforeSolving.getTaskDate(currentTask);
			if (useCalDate && currentTask.getTentativeStartDate() != null) {
				taskDate = currentTask.getTentativeStartDate();
			}
			calendar.setTime(taskDate);
	...
	public static List<ProjectTask> getCurrentTasks(Project project, Date from, Date to , boolean useCalDate) {
		...
		for (ProjectTask currentTask : projectTasks) {
			...
			calendar.setTime(taskDate);
			calendar.add(Calendar.DATE, calculateTaskDuration(currentTask) - 1);
			Date taskEndDate = calendar.getTime();
			// From or To?
	...
	public static double getPortfolioOpenBalanceNew(HttpSession session, Portfolio portfolio, Date from) {
		...
		for (Project project : projects) {
			try {
				Date projectStartDate = PaymentUtilBeforeSolving.getProjectDateRanges(controller, project.getProjectId())[0];
				if (projectStartDate != null) { // has some tasks or payments.
					cashFlowInfo = PaymentUtilBeforeSolving.getProjectCashFlowDataNew(session, project.getProjectId(), projectStartDate , calendar.getTime());
					DailyCashFlowMapEntity previousDay = PaymentUtilBeforeSolving.getPreviousDayInfo(from , cashFlowInfo, project.getProjectId());
	...
	public static double getPortfolioOpenBalance(HttpSession session, Portfolio portfolio, Date from) {
		...
		for (Project project : projects) {
			try {
				Date projectStartDate = PaymentUtilBeforeSolving.getProjectDateRanges(controller, project.getProjectId())[0];
				if (projectStartDate != null) { // has some tasks or payments.
					cashFlowInfo = PaymentUtilBeforeSolving.getProjectCashFlowData(session, project.getProjectId(), projectStartDate , calendar.getTime());
					DailyCashFlowMapEntity previousDay = PaymentUtilBeforeSolving.getPreviousDayInfo(from , cashFlowInfo, project.getProjectId());
					balanceAccumulator += previousDay == null? 0 : previousDay.getBalance() ;
				}
			} catch (EntityControllerException | OptimaException e) {
	...
	public static double getPortfolioFinanceCostNew(HttpSession session, Portfolio portfolio, Date from, Date to) throws EntityControllerException, OptimaException {
		...
		List<Project> projects = portfolio.getProjects();
		for (Project currentProject : projects) {
			Date projectStartDate = PaymentUtilBeforeSolving.getProjectDateRanges(controller, currentProject.getProjectId())[0];
			if (projectStartDate != null) {
				cashFlowInfo = PaymentUtilBeforeSolving.getProjectCashFlowDataNew(session, currentProject.getProjectId(), projectStartDate , calendar.getTime());
	
				Calendar prevDay = Calendar.getInstance();
				prevDay.setTime(from);
				prevDay.add(Calendar.DATE, -1);
				
				DailyCashFlowMapEntity previousDay = PaymentUtilBeforeSolving.getPreviousDayInfo(from , cashFlowInfo, currentProject.getProjectId());
				
				double firstDateFinanceCost = PaymentUtilBeforeSolving.getDateFinanceCost(currentProject, from, cashFlowInfo); 
				financeCostCounter += firstDateFinanceCost;
				double firstDayBalance = previousDay == null? 0 : previousDay.getBalance() ;
				double projectPayment = PaymentUtilBeforeSolving.getProjectPaymentstNew(session, currentProject, from);
				...
				for (Date date = start.getTime() ; start.before(end); start.add(Calendar.DATE, 1), date = start.getTime()) {
					...
					if (!PaymentUtilBeforeSolving.isDayOff(date,currentProject.getDaysOffs()) && ! PaymentUtilBeforeSolving.isWeekendDay(date,currentProject.getWeekendDays())) {
						tasks = currentProject.getProjectTasks();
						for (ProjectTask currentTask : tasks) {
					
							taskDate = PaymentUtilBeforeSolving.getTaskDate(currentTask);
							calendar.setTime(taskDate);
							calendar.add(Calendar.DATE, calculateTaskDuration(currentTask) - 1);
	...
	public static double getPortfolioFinanceCost(HttpSession session, Portfolio portfolio, Date from, Date to) throws EntityControllerException, OptimaException {
		...
		for (Project currentProject : projects) {
			Date projectStartDate = PaymentUtilBeforeSolving.getProjectDateRanges(controller, currentProject.getProjectId())[0];
			if (projectStartDate != null) {
				cashFlowInfo = PaymentUtilBeforeSolving.getProjectCashFlowData(session, currentProject.getProjectId(), projectStartDate , calendar.getTime());
				...
				financeCostCounter += firstDateFinanceCost;
				double firstDayBalance = previousDay == null? 0 : previousDay.getBalance() ;
				double projectPayment = PaymentUtilBeforeSolving.getProjectPaymentst(currentProject, from);
				firstDayBalance += projectPayment;
				balanceCounter = firstDayBalance;
	...
	public static double getProjectOverhead(Project project, Date from, Date to) {
		int daysInPeriod = PaymentUtilBeforeSolving.daysBetween(from, to);	
	...
	public static double getEligiableTaskCost( List<ProjectTask> eligibleTasks , Date to ) {
		...
		for (ProjectTask task : eligibleTasks) {
			...
			endDate.add(Calendar.DATE, calculateTaskDuration(task));
			
			int taskLength = PaymentUtilBeforeSolving.daysBetween(taskDate , endDate.getTime());
			int daysInPeriod = PaymentUtilBeforeSolving.daysBetween(taskDate , to);
			...
			int noOfWeekendDaysAndDaysOf = PaymentUtilBeforeSolving.getNoOfWeekEndDaysAndDaysOff(task, taskDate, dateToCompare); // What if taskEndDate > to?
	...
	public static double getEligiableTaskCurrentPeriodCost( List<ProjectTask> eligibleTasks , Date start, Date end) {
		...
		Calendar calendar = Calendar.getInstance();
		for (ProjectTask task : eligibleTasks) {
			Date taskDate = task.getTentativeStartDate();
			int daysInPeriod = PaymentUtilBeforeSolving.daysBetween(taskDate, end);			
			int effictiveNumberOfDays = Math.min(calculateTaskDuration(task) , daysInPeriod);
			if (effictiveNumberOfDays == calculateTaskDuration(task)) {
				calendar.setTime(taskDate);
				calendar.add(Calendar.DATE, calculateTaskDuration(task) - 1);
			} else {
				calendar.setTime(end);
			}
			int noOfWeekendDaysAndDaysOf = PaymentUtilBeforeSolving.getNoOfWeekEndDaysAndDaysOff(task, taskDate, calendar.getTime()); // What if taskEndDate > to?
	...
	public static double getOtherProjectsCurrentPeriodCost( Project project, Date start, Date end) {
		...
		for (Project currentProject : projects) {
			if (currentProject.getProjectId() != project.getProjectId()) {
				List<ProjectTask> eligibleTasks = PaymentUtilBeforeSolving.getEligibleTasks(currentProject, start, end , false);
				costAccumulator += PaymentUtilBeforeSolving.getEligiableTaskCurrentPeriodCost(eligibleTasks , start, end);
			}
		}
		return costAccumulator;
	}
			...
			if (currentProject.getProjectId() != project.getProjectId()) {
				...
				for (ProjectPayment payment: currentProject.getProjectPayments()) {
					if (payment.getPaymentDate().equals(end)) {
						...
						for (ProjectTask task : eligibleTasks) {
							Date taskDate = task.getTentativeStartDate();
							int taskLength = calculateTaskDuration(task);
							calendar.setTime(taskDate);
							calendar.add(Calendar.DATE, taskLength);
							Date taskEndDate = calendar.getTime();
							if (taskEndDate.getTime() >= end.getTime()) {
								int effictiveNumberOfDays = PaymentUtilBeforeSolving.daysBetween(end, taskEndDate);
								if(maxEffictiveNumberOfDays < effictiveNumberOfDays)
									maxEffictiveNumberOfDays = effictiveNumberOfDays;
								int noOfWeekendDaysAndDaysOf = PaymentUtilBeforeSolving.getNoOfWeekEndDaysAndDaysOff(task, end, taskEndDate); // What if taskEndDate > to?
								effictiveNumberOfDays -= noOfWeekendDaysAndDaysOf;
								costAccumulator += effictiveNumberOfDays * task.getUniformDailyCost().doubleValue();
							}
						}
						...
						if(projId !=null)
						{
							...
							overheadDays = PaymentUtilBeforeSolving.daysBetween(end, PaymentUtilBeforeSolving.getNextEventNew(session, project , end, projectExpectedEndDate));
			...
			if (currentProject.getProjectId() != project.getProjectId()) {
				for (ProjectPayment payment: currentProject.getProjectPayments()) {
					if (payment.getPaymentDate().equals(end)) {
						...
						int overheadDays = PaymentUtilBeforeSolving.daysBetween(end, PaymentUtilBeforeSolving.getNextEvent(project , end));
						costAccumulator += overheadDays * project.getOverheadPerDay().doubleValue();
						break;
					}		
				}
	...
	public static double getEligiableTaskLeftOverCostNew(HttpSession session, List<ProjectTask> eligibleTasks ,Project project, Date start, Date end,  Date projectExpectedEndDate) throws EntityControllerException {
		double costAccumulator = 0;
		Calendar calendar = Calendar.getInstance();
		Date nextEventDate = PaymentUtilBeforeSolving.getNextEventNew(session, project , end, projectExpectedEndDate);
		for (ProjectTask task : eligibleTasks) {
			Date taskDate = task.getTentativeStartDate();
			int taskLength = calculateTaskDuration(task);
			calendar.setTime(taskDate);
			...
			if (taskEndDate.getTime() >= end.getTime()) {
				...
				int effictiveNumberOfDays = PaymentUtilBeforeSolving.daysBetween(end, endOfEffectiveDays);
				int noOfWeekendDaysAndDaysOf = PaymentUtilBeforeSolving.getNoOfWeekEndDaysAndDaysOff(task, end, endOfEffectiveDays); // What if taskEndDate > to?
				effictiveNumberOfDays -= noOfWeekendDaysAndDaysOf;
				costAccumulator += effictiveNumberOfDays * task.getUniformDailyCost().doubleValue();
			}
		}
		int overheadDays = PaymentUtilBeforeSolving.daysBetween(end, nextEventDate);
		costAccumulator += overheadDays * project.getOverheadPerDay().doubleValue();
		return costAccumulator;
	}
	...
	public static double getEligiableTaskLeftOverCost( List<ProjectTask> eligibleTasks ,Project project, Date start, Date end) {
		double costAccumulator = 0;
		Calendar calendar = Calendar.getInstance();
		Date nextEventDate = PaymentUtilBeforeSolving.getNextEvent(project , end);
		for (ProjectTask task : eligibleTasks) {
			Date taskDate = task.getTentativeStartDate();
			int taskLength = calculateTaskDuration(task);
			calendar.setTime(taskDate);
	...
	private static Date getNextEventNew(HttpSession session, Project project, Date end, Date projectExpectedEndDate) throws EntityControllerException {
		...
		for (ProjectPayment payment : projectPayments) {
			...
			if (paymentDate.getTime() > end.getTime()) {
				int startsAfterTmp = PaymentUtilBeforeSolving.daysBetween(end, paymentDate);
		...
		for (PortfolioFinance finance : project.getPortfolio().getPortfolioFinances()) {
			...
			if (financeDate.getTime() > end.getTime()) {
				int startsAfterTmp = PaymentUtilBeforeSolving.daysBetween(end, financeDate);
	...
	public static double getEligiableTaskFinanceCostNew(HttpSession session , Project project,
		...
		Map<String, DailyCashFlowMapEntity> cashFlowInfo = PaymentUtilBeforeSolving.getProjectCashFlowData(session, project.getProjectId(), projectStartDate , firstDateCal.getTime());
		
		DailyCashFlowMapEntity previousDay = PaymentUtilBeforeSolving.getPreviousDayInfo(from, cashFlowInfo, project.getProjectId());
		
		Calendar calendar = Calendar.getInstance();

		double firstDateFinanceCost = PaymentUtilBeforeSolving.getDateFinanceCost(project, from, cashFlowInfo); 
		...
		double firstDayBalance = previousDay == null? 0 : previousDay.getBalance() ;
		firstDayBalance += PaymentUtilBeforeSolving.getProjectPaymentstNew(session, project, from);
		...
		for (Date date = start.getTime() ; start.before(end); start.add(Calendar.DATE, 1), date = start.getTime()) {
			if (!PaymentUtilBeforeSolving.isDayOff(date,project.getDaysOffs()) && ! PaymentUtilBeforeSolving.isWeekendDay(date,project.getWeekendDays())) {
	...
	public static double getEligiableTaskFinanceCost(HttpSession session , Project project,
		...
		double firstDayBalance = previousDay == null? 0 : previousDay.getBalance() ;
		firstDayBalance += PaymentUtilBeforeSolving.getProjectPaymentst(project, from);
		double balanceCounterEligibleTasks = firstDayBalance;
		double balanceCounterLeftovers = firstDayBalance;
	...
	public static void processTask(ProjectTask task, Project project)  {
		...
		for (TaskDependency dependency : task.getAsDependency()) {
			...
			if (nextTask != null) {
				Date nextTaskStartDate = nextTask.getTentativeStartDate();
				...
				if (nextTaskStartDate == null || nextTaskStartDate.before(cal.getTime())) {
					...
					while (PaymentUtilBeforeSolving.isDayOff(cal.getTime(), project.getDaysOffs())  
							|| PaymentUtilBeforeSolving.isWeekendDay(cal.getTime() ,  project.getWeekendDays()) )
					{
	...
	public static void calculateCalederDuration(Project project, ProjectTask task) {
		Date startDate = task.getTentativeStartDate();
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(startDate);
		int duration = task.getDuration();
		int calendarDuration = 0;
		while (duration > 0) {
			if ( PaymentUtilBeforeSolving.isDayOff(startDate, project.getDaysOffs())
					||  PaymentUtilBeforeSolving.isWeekendDay(startDate, project.getWeekendDays())) {
				calendarDuration++;
			} else {
				duration--;
				calendarDuration++;
	...
	public static int getProjectLength(Project project , String outputType , Logger logger) {
		...
		Date minStartDate = tasks.get(0).getTentativeStartDate(); // getTaskDate(tasks.get(0)); //
		Calendar calendar = Calendar.getInstance();
		...
		for (ProjectTask currentTask : tasks) {
			...
			Date taskStartDate = currentTask.getTentativeStartDate();  // 	getTaskDate(currentTask);
		...
		int numberOfDays = PaymentUtilBeforeSolving.daysBetween(minStartDate, maxEndDate);
		if (outputType.equals("detailedOutput")) {
	...
	public static TaskSolution findBestIterationNew(List<TaskSolution> solutions, double cashAvailable, double cashAvailableNextPeriod) {
		...
		solution = PaymentUtilBeforeSolving.findBestFeasbileIteration(solutions, cashAvailable, cashAvailableNextPeriod);
		if (solution == null) {
			solution = PaymentUtilBeforeSolving.findBestIteration(solutions);
		}
		return solution;
	}
			...
			for (int i = 0 ; i < solvedProjectsArray.length; i++) {
				...
				for (Project currentProject : projects) {
					if (currentProject.getProjectId() == projectId) {
						List<ProjectTask> eligibleTasks = PaymentUtilBeforeSolving.getEligibleTasks(currentProject, from, to , false);
						cost += PaymentUtilBeforeSolving.getEligiableTaskCost(eligibleTasks , to);
					}
				}
			}
		}
	...
	private static double getExpectedPaymentNoAdjustments(Project project,
		...
		for (ProjectTask currentTask : projectTasks) {
			...
			if ((taskDate.getTime() < paymentEnd.getTime()  &&  taskEndDate.getTime() >= paymentStart.getTime())) {
				...
				effictiveNumberOfDays = PaymentUtilBeforeSolving.daysBetween(startDate , endDate) + (endDate.equals(paymentEnd)?0:1);
				 
				
				
				int noOfWeekendDaysAndDaysOf = PaymentUtilBeforeSolving.getNoOfWeekEndDaysAndDaysOff(currentTask, startDate, endDate); // What if taskEndDate > to?
		...
		for (Project project : portfolio.getProjects()) {
			...
			for (ProjectPayment payment: project.getProjectPayments()) {
				...
				if (!doNotCalculate && payment.getPaymentType().getPaymentType().equalsIgnoreCase("Intrim") && payment.getPaymentDate().equals(from) && payment.getPaymentAmount().doubleValue() == 0) {
					if (detail != null) {
						expectedPayment += PaymentUtilBeforeSolving.getExpectedPayment(project,  detail.getPaymentStart(),  detail.getPaymentEnd(), to,  detail.getExtra(), 
	...
	public static double getPortfolioPaymentNew(HttpSession session, Date from , Date to, Map<Integer , ProjectPaymentDetail> paymentDetails , Portfolio portfolio, boolean doNotCalculate, Date[] projectBoundaries) throws EntityControllerException {
		...
		for (Project project : portfolio.getProjects()) {
			...
			for (ProjectPayment payment: projectPayments) {
				...
				if (!doNotCalculate && payment.getPaymentType().getPaymentType().equalsIgnoreCase("Intrim") && payment.getPaymentDate().equals(from) && payment.getPaymentAmount().doubleValue() == 0) {
					if (detail != null) {
						expectedPayment += PaymentUtilBeforeSolving.getExpectedPaymentNew(session, project,  detail.getPaymentStart(),  detail.getPaymentEnd(), to,  detail.getExtra(), 
	...
	public static double getProjectPayment(Date from, Date to, double advanceRepayment,
		...
		for (ProjectPayment payment: project.getProjectPayments()) {
			...
			if (payment.getPaymentType().getPaymentType().equalsIgnoreCase("Intrim") && payment.getPaymentDate().equals(from) && payment.getPaymentAmount().doubleValue() == 0) {
				expectedPayment += PaymentUtilBeforeSolving.getExpectedPayment(project, paymentStart, paymentEnd , to,  extraPayment, retainPercent, advanceRepayment);
			} else if (payment.getPaymentDate().equals(from)) {
				expectedPayment += payment.getPaymentAmount().doubleValue();
			}
		}
	...
	public static double getProjectPaymentNoAdjustments(Date from, Date to, Date paymentStart, Date paymentEnd ,
		...
		for (ProjectPayment payment: project.getProjectPayments()) {
			...
			if (payment.getPaymentType().getPaymentType().equalsIgnoreCase("Intrim") && payment.getPaymentDate().equals(from) && payment.getPaymentAmount().doubleValue() == 0) {
				expectedPayment += PaymentUtilBeforeSolving.getExpectedPaymentNoAdjustments(project, paymentStart, paymentEnd);
			} else if (payment.getPaymentDate().equals(from)) {
				expectedPayment += payment.getPaymentAmount().doubleValue();
			}
		}
	...
	public static double getNextPeriodLeftOverFinanceCost(double openBalance, Date to,
		...
		for ( ProjectTask task : eligibleTasks) {
			Date taskDate = PaymentUtilBeforeSolving.getTaskDate(task);
			calendar.setTime(taskDate);
	...
	public static Object getTaskListStart(List<ProjectTask> tasks) {
		...
		Calendar calendar = Calendar.getInstance();
		for (ProjectTask task : tasks) {
			calendar.setTime(task.getTentativeStartDate());
	...
	public static Map<Integer, ProjectPaymentDetail> getPaymentDetailsNew(HttpSession session , int projectId, Date from, Date to) {
		...
		try {
			...
			Project project = controller.find(Project.class , projectId);
			Date[] projectBoundaries = PaymentUtilBeforeSolving.getPortofolioDateRanges(controller, project.getPortfolio().getPortfolioId());
